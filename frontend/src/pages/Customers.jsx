...
const Customers = () => {
    ...
    const load = useCallback(() => {
        setLoading(true);
        API.get('/customers', { params: { search, page, size: 10 } })
    ...
    const del = (id) => {
        ...
        API.delete(`/customers/${id}`).then(load).catch(() => setError('Delete failed'));
    };
    ...
    const columns = [
        ...
        { key: 'creditLimit', label: 'Credit Limit', render: r => r.creditLimit ? `$${Number(r.creditLimit).toFixed(2)}` : '—' },
        { key: 'totalDebt', label: 'Total Debt', render: r => <span style={{ color: r.totalDebt > 0 ? '#ef4444' : '#22c55e' }}>${Number(r.totalDebt || 0).toFixed(2)}</span> },
    ];
    ...
    return (
        <div className="page-container">
            ...
            <FormDialog open={dialog} onClose={() => setDialog(false)} title={editId ? 'Edit Customer' : 'New Customer'}
                footer={<><button className="btn-secondary" onClick={() => setDialog(false)}>Cancel</button><button className="btn-primary" form="cust-form" type="submit">Save</button></>}>
                <form id="cust-form" onSubmit={submit} className="form-grid">
